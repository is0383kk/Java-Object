...
# 静的メンバ  
インスタンスを生成すると、インスタンス毎に固有の値が保持される。  
例えば、山田インスタンスの「salary」と吉田インスタンスの「salary」は固有の値を持つ。
ここで、山田と吉田のインスタンスで「salary」の値を共有させたい場合、<strong>クラス変数</strong>を使う。

- クラス変数：クラス内で共有したいフィールド値にstaticをつける  
